...
class AuthenticationProvider {
  ...
  Future<Map> signIn(GoogleSignInAccount googleUser) async {
    ...
    res = await databaseProvider.createUser(signedInUser.uid, signedInUser.displayName, signedInUser.photoURL);
    return res;
  }
    ...
    if (googleUser != null) {
      ...
      await firestore.collection('user-data').get().then((value) {
        value.docs.forEach((element) async {
          if (Users.fromJson(element.data()).friends.indexWhere((user) => user.id == userToDelete.id) != -1) {
            Users friend = Users.fromJson(element.data());
            friend.friends.removeAt(friend.friends.indexWhere((user) => user.id == userToDelete.id));
            await firestore.collection("user-data").doc(element.id).set(friend.toJson(), SetOptions(merge: true));
          }
        });
      });
      ...
      await firestore.collection('requests').get().then((value) {
        value.docs.forEach((element) async {
          ...
          if (request.requestee.id == userToDelete.id || request.requester.id == userToDelete.id) {
